...
public class PlayerController : MonoBehaviour {
	...
	void GenerateEnemy(){
		...
		Instantiate (
			enemy,
			new Vector3 (Random.Range (-100, 100), transform.position.y, transform.position.z + 200),
			q
		);
		Instantiate(
			enemy,
			new Vector3(transform.position.x, transform.position.y, transform.position.z + 200),
			q
		);
	}
